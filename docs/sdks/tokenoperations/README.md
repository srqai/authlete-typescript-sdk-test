...
async function run() {
  const result = await authlete.tokenOperations.listTokens({
    serviceId: "<id>",
  });
...
import { tokenOperationsListTokens } from "authlete/funcs/tokenOperationsListTokens.js";
...
async function run() {
  const res = await tokenOperationsListTokens(authlete, {
    serviceId: "<id>",
  });
  if (res.ok) {
...
async function run() {
  const result = await authlete.tokenOperations.update({
    serviceId: "<id>",
    tokenUpdateRequest: {
...
```typescript
import { AuthleteCore } from "authlete/core.js";
import { tokenOperationsUpdate } from "authlete/funcs/tokenOperationsUpdate.js";
...
async function run() {
  const res = await tokenOperationsUpdate(authlete, {
    serviceId: "<id>",
    tokenUpdateRequest: {
      accessToken: "Z5a40U6dWvw2gMoCOAFbZcM85q4HC0Z--0YKD9-Nf6Q",
      scopes: [
