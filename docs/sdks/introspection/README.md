...
* [request](#request) - Process Introspection Request
...
async function run() {
  const result = await authlete.introspection.request({
    serviceId: "<id>",
...
import { introspectionRequest } from "authlete/funcs/introspectionRequest.js";
...
async function run() {
  const res = await introspectionRequest(authlete, {
    serviceId: "<id>",
    introspectionRequest: {
      token: "VFGsNK-5sXiqterdaR7b5QbRX9VTwVCQB87jbr2_xAI",
      scopes: [
  ...
  if (res.ok) {
    ...
    console.log("introspectionRequest failed:", res.error);
  }
}
