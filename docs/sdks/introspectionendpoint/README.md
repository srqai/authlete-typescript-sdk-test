...
* [processStandard](#processstandard) - Process OAuth 2.0 Introspection Request
...
async function run() {
  const result = await authlete.introspectionEndpoint.processStandard({
    serviceId: "<id>",
...
import { introspectionEndpointProcessStandard } from "authlete/funcs/introspectionEndpointProcessStandard.js";
...
async function run() {
  const res = await introspectionEndpointProcessStandard(authlete, {
    serviceId: "<id>",
    standardIntrospectionRequest: {
      parameters: "token=VFGsNK-5sXiqterdaR7b5QbRX9VTwVCQB87jbr2_xAI&token_type_hint=access_token",
    },
  ...
  if (res.ok) {
    ...
    console.log("introspectionEndpointProcessStandard failed:", res.error);
  }
}
