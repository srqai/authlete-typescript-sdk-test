...
async function run() {
  const result = await authlete.introspectionEndpoint.processIntrospection({
    serviceId: "<id>",
...
import { introspectionEndpointProcessIntrospection } from "authlete/funcs/introspectionEndpointProcessIntrospection.js";
...
async function run() {
  const res = await introspectionEndpointProcessIntrospection(authlete, {
    serviceId: "<id>",
    standardIntrospectionRequest: {
      parameters: "token=VFGsNK-5sXiqterdaR7b5QbRX9VTwVCQB87jbr2_xAI&token_type_hint=access_token",
    },
