...
* [getList](#getlist) - List Security Keys
...
import { hardwareSecurityKeysCreate } from "authlete/funcs/hardwareSecurityKeysCreate.js";
...
async function run() {
  const result = await authlete.hardwareSecurityKeys.getList({
    serviceId: "<id>",
  });
...
```typescript
import { AuthleteCore } from "authlete/core.js";
import { hardwareSecurityKeysGetList } from "authlete/funcs/hardwareSecurityKeysGetList.js";
...
async function run() {
  const res = await hardwareSecurityKeysGetList(authlete, {
    serviceId: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("hardwareSecurityKeysGetList failed:", res.error);
  }
}
