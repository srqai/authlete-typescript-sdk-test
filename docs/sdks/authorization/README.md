...
* [ticketUpdate](#ticketupdate) - Update Ticket Information
...
async function run() {
  const result = await authleteTest.authorization.ticketUpdate({
    serviceId: "<id>",
...
import { authorizationTicketUpdate } from "authlete-test/funcs/authorizationTicketUpdate.js";
...
async function run() {
  const res = await authorizationTicketUpdate(authleteTest, {
    serviceId: "<id>",
    authorizationTicketUpdateRequest: {
      ticket: "<value>",
      info: "<value>",
  ...
  if (res.ok) {
    ...
    console.log("authorizationTicketUpdate failed:", res.error);
  }
}
