...
* [process](#process) - Process Authorization Request
...
async function run() {
  const result = await authlete.authorization.process({
    serviceId: "<id>",
...
import { authorizationProcess } from "authlete/funcs/authorizationProcess.js";
...
async function run() {
  const res = await authorizationProcess(authlete, {
    serviceId: "<id>",
    authorizationRequest: {
      parameters: "response_type=code&client_id=26478243745571&redirect_uri=https%3A%2F%2Fmy-client.example.com%2Fcb1&scope=timeline.read+history.read&code_challenge=E9Melhoa2OwvFrEMTJguCHaoeK1t8URWbuGJSstw-cM&code_challenge_method=S256",
    },
  ...
  if (res.ok) {
    ...
    console.log("authorizationProcess failed:", res.error);
  }
}
