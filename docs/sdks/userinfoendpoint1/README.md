...
async function run() {
  const result = await authleteTest.userinfoEndpoint.processUserInfo({
    serviceId: "<id>",
    userinfoRequest: {
...
import { userinfoEndpointProcessUserInfo } from "authlete-test/funcs/userinfoEndpointProcessUserInfo.js";
...
async function run() {
  const res = await userinfoEndpointProcessUserInfo(authleteTest, {
    serviceId: "<id>",
    userinfoRequest: {
      token: "Ntm9MDb8WXQAevqrBkd84KTTHbYHVQrTjgUZCOWqEUI",
    },
