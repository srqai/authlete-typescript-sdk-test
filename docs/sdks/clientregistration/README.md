...
async function run() {
  const result = await authleteTest.clientRegistration.register({
    serviceId: "<id>",
    requestBody: {
...
import { clientRegistrationRegister } from "authlete-test/funcs/clientRegistrationRegister.js";
...
async function run() {
  const res = await clientRegistrationRegister(authleteTest, {
    serviceId: "<id>",
    requestBody: {
      json: "{ \"client_name\": \"My Dynamic Client\" }",
    },
...
async function run() {
  const result = await authleteTest.clientRegistration.get({
    serviceId: "<id>",
    requestBody: {
...
```typescript
import { AuthleteTestCore } from "authlete-test/core.js";
import { clientRegistrationGet } from "authlete-test/funcs/clientRegistrationGet.js";
...
async function run() {
  const res = await clientRegistrationGet(authleteTest, {
    serviceId: "<id>",
    requestBody: {
      token: "qs4Tu5TV7qqDYT93bFs6ISyhTByMF9o-54GY4JU5vTA",
      clientId: "26837717140341",
