...
async function run() {
  const result = await authlete.tokenEndpoint.processToken({
    serviceId: "<id>",
    tokenRequest: {
...
import { tokenEndpointProcessToken } from "authlete/funcs/tokenEndpointProcessToken.js";
...
async function run() {
  const res = await tokenEndpointProcessToken(authlete, {
    serviceId: "<id>",
    tokenRequest: {
      parameters: "grant_type=authorization_code&code=Xv_su944auuBgc5mfUnxXayiiQU9Z4-T_Yae_UfExmo&redirect_uri=https%3A%2F%2Fmy-client.example.com%2Fcb1&code_verifier=dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk",
      clientId: "26478243745571",
...
async function run() {
  const result = await authlete.tokenEndpoint.issueToken({
    serviceId: "<id>",
    tokenIssueRequest: {
...
```typescript
import { AuthleteCore } from "authlete/core.js";
import { tokenEndpointIssueToken } from "authlete/funcs/tokenEndpointIssueToken.js";
...
async function run() {
  const res = await tokenEndpointIssueToken(authlete, {
    serviceId: "<id>",
    tokenIssueRequest: {
      ticket: "p7SXQ9JFjng7KFOZdCMBKcoR3ift7B54l1LGIgQXqEM",
      subject: "john",
...
async function run() {
  const result = await authlete.tokenEndpoint.reissueIdToken({
    serviceId: "<id>",
  });
...
```typescript
import { AuthleteCore } from "authlete/core.js";
import { tokenEndpointReissueIdToken } from "authlete/funcs/tokenEndpointReissueIdToken.js";
...
async function run() {
  const res = await tokenEndpointReissueIdToken(authlete, {
    serviceId: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
