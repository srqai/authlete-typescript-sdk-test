...
async function run() {
  const result = await authlete.services.list();
...
import { servicesList } from "authlete/funcs/servicesList.js";
...
async function run() {
  const res = await servicesList(authlete);
  if (res.ok) {
...
async function run() {
  const result = await authlete.services.create({
...
```typescript
import { AuthleteCore } from "authlete/core.js";
import { servicesCreate } from "authlete/funcs/servicesCreate.js";
...
async function run() {
  const res = await servicesCreate(authlete, {
    serviceName: "My service",
    issuer: "https://my-service.example.com",
    clientIdAliasEnabled: true,
    supportedGrantTypes: [
...
async function run() {
  await authlete.services.delete({
    serviceId: "<id>",
  });
...
```typescript
import { AuthleteCore } from "authlete/core.js";
import { servicesDelete } from "authlete/funcs/servicesDelete.js";
...
async function run() {
  const res = await servicesDelete(authlete, {
    serviceId: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
...
async function run() {
  const result = await authlete.services.configuration({
    serviceId: "<id>",
  });
...
```typescript
import { AuthleteCore } from "authlete/core.js";
import { servicesConfiguration } from "authlete/funcs/servicesConfiguration.js";
...
async function run() {
  const res = await servicesConfiguration(authlete, {
    serviceId: "<id>",
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("servicesConfiguration failed:", res.error);
  }
}
