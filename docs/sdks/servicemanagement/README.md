...
async function run() {
  const result = await authlete.service.management.getConfiguration({
    serviceId: "<id>",
  });
...
import { serviceManagementGetConfiguration } from "authlete/funcs/serviceManagementGetConfiguration.js";
...
async function run() {
  const res = await serviceManagementGetConfiguration(authlete, {
    serviceId: "<id>",
  });
  if (res.ok) {
