...
async function run() {
  const result = await authlete.utilityEndpoints.getInfo();
...
import { utilityEndpointsGetInfo } from "authlete/funcs/utilityEndpointsGetInfo.js";
...
async function run() {
  const res = await utilityEndpointsGetInfo(authlete);
  if (res.ok) {
