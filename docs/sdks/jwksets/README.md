...
async function run() {
  const result = await authlete.jwkSets.get({
    serviceId: "<id>",
  });
...
import { jwkSetsGet } from "authlete/funcs/jwkSetsGet.js";
...
async function run() {
  const res = await jwkSetsGet(authlete, {
    serviceId: "<id>",
  });
  if (res.ok) {
    ...
    console.log("jwkSetsGet failed:", res.error);
  }
}
