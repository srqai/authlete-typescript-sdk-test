...
```typescript
import { AuthleteCore } from "authlete/core.js";
import { utilityEcho } from "authlete/funcs/utilityEcho.js";
...
async function run() {
  const res = await utilityEcho(authlete);
  if (res.ok) {
    const { value: result } = res;
    
  } else {
    console.log("utilityEcho failed:", res.error);
  }
}
