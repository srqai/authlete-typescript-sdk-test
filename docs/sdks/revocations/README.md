...
async function run() {
  const result = await authlete.revocations.process({
    serviceId: "<id>",
    revocationRequest: {
...
import { revocationsProcess } from "authlete/funcs/revocationsProcess.js";
...
async function run() {
  const res = await revocationsProcess(authlete, {
    serviceId: "<id>",
    revocationRequest: {
      parameters: "VFGsNK-5sXiqterdaR7b5QbRX9VTwVCQB87jbr2_xAI&token_type_hint=access_token",
      clientId: "26478243745571",
  ...
  if (res.ok) {
    ...
    console.log("revocationsProcess failed:", res.error);
  }
}
