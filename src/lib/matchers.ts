...
import { AuthleteTestDefaultError } from "../models/errors/authletetestdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | AuthleteTestDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | AuthleteTestDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new AuthleteTestDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new AuthleteTestDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
