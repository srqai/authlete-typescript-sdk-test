...
import { AuthleteDefaultError } from "../models/errors/authletedefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | AuthleteDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | AuthleteDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new AuthleteDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new AuthleteDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
